...
export default function create() {
  ...
  const updateForm = (field: Partial<FormState>) => {
  ...
  const handlePickImages = async () => {
    ...
    for (const asset of result.assets) {
      try {
        ...
        const { data: urlData } = await authSupabase.storage
          .from("property-images")
  ...
  const handleDetectLocation = async () => {
    ...
    try {
      ...
      Alert.alert("Error", "There was an error detecting your location.");
    } finally {
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      >
        {/* Header */}
        <View className="flex-row items-center  px-4 py-3 pb-3 border-gray-200">
        ...
        >
          <View className={sectionClass}>
            ...
            <View className="flex-row flex-wrap gap-3">
              ...
              {form.localImages.length < 6 && (
                <TouchableOpacity
                  className="w-24 h-24 bg-white rounded-2xl border-dashed border-2 border-gray-300 items-center justify-center"
          ...
          {/* Basic Info */}
          <View className={sectionClass}>
            <Text className={labelClass}>Basic Information</Text>
            ...
            <View className={sectionClass}>
              ...
              <TextInput
                className={inputClass}
                placeholderTextColor="#9CA3AF"
                placeholder="e.g. A beautiful 2-bedroom apartment located in the heart of the city..."
                  ...
                  >
                    <Text
                      className={`text-sm font-medium ${
                        form.type === type ? "text-white" : "text-gray-700"
                      }`}
                    >
                      {type}
                    </Text>
...
const Toggle = ({
  ...
  >
    <View className="flex-1 mr-3">
      <Text
        className={`font-semibold ${value ? "text-blue-600" : "text-gray-700"}`}
      >
        {label}
      </Text>
      {description && (
    ...
    <View
      className={`w-10 h-6 rounded-full border-2 items-center justify-center ${
        value ? "bg-blue-500 border-blue-500" : "bg-gray-300 border-gray-300"
      }`}
    >
